...
//   const [departmentSuggestions, setDepartmentSuggestions] = useState(['General', 'Cardiology', 'Pediatrics']);
//   const [showSuggestions, setShowSuggestions] = useState(false);
//   const [configData, setConfigData] = useState([]);
//   const [editingId, setEditingId] = useState(null);
//   const [isAddingNew, setIsAddingNew] = useState(false);
//   const [locationSuggestions, setLocationSuggestions] = useState(['Main Office']);
...
//       setIsLoading(true);
//       try {
//         const response = await axios.get('http://localhost:5001/api/slotsconfig/all');
        
//         // Handle different possible response structures
//         const data = response.data?.data || response.data?.configurations || [];
...
//             departments.length ? departments : [...prev]
//           );
//           setLocationSuggestions(prev => 
//             locations.length ? locations : [...prev]
//           );
//         } else {
//           console.error('Unexpected API response format:', response.data);
//           setMessage('Failed to load configurations: Invalid data format');
//         }
//       } catch (err) {
//         console.error('Failed to fetch initial data:', err);
//         setMessage(err.response?.data?.message || 'Failed to load configurations');
//       } finally {
//         setIsLoading(false);
//       }
//     };
...
//     // Validate inputs
//     if (!config.max_slots || config.max_slots < 1) {
//       setMessage('Please enter a valid number of slots (minimum 1)');
//       return;
//     }
...
//     setIsLoading(true);
//     try {
//       const endpoint = 'http://localhost:5001/api/slotsconfig';
//       const payload = {
//         max_slots: Number(config.max_slots),
//         location: config.location.trim(),
//         department: config.department.trim()
//       };
...
//         setConfigData(allConfigs.data.configurations);
//         // Reset form
//         setConfig({ max_slots: 20, location: 'Main Office', department: 'General' });
//         setIsAddingNew(true);
//       }
//     } catch (err) {
...
//         // Safely fetch and update configurations
//         const allConfigs = await axios.get('http://localhost:5001/api/slotsconfig/all');
        
//         // Add null checks and fallbacks
//         const configurations = allConfigs.data?.configurations || [];
//         setConfigData(configurations);
        
//         // Update suggestions with proper error handling
//         const departments = [...new Set(configurations.map(c => c?.department).filter(Boolean))];
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        // Handle different possible response structures
        const data = response.data?.data || response.data?.configurations || response.data || [];
        
        if (Array.isArray(data)) {
          setConfigData(data);
          
          // Extract unique departments and locations
          const departments = [...new Set(data.map(c => c?.department).filter(Boolean))];
          const locations = [...new Set(data.map(c => c?.location).filter(Boolean))];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Refresh the configuration list
        const allConfigs = await axios.get('http://localhost:5001/api/slotsconfig/all');
        const configurations = allConfigs.data?.data || allConfigs.data?.configurations || allConfigs.data || [];
        setConfigData(configurations);
            ...
            <table className="min-w-full bg-white border border-gray-200">
              ...
              <tbody>
                {configData?.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="5" className="py-4 text-center text-gray-500">
                      {isLoading ? 'Loading configurations...' : 'No configuration data available'}
                    </td>
                  </tr>
                )}
              </tbody>
