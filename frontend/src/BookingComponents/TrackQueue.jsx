...
const TrackQueue = ({ userId }) => {
  ...
  const fetchQueue = useCallback(async (signal) => {
    try {
      ...
      const response = await axios.get(`${BACKEND_URL}/slots`, {
        signal,
        headers: { 
      ...
      if (!axios.isCancel(err)) {
        console.error('Failed to fetch queue:', err);
        setError(`API Error: ${err.message}. URL: ${err.config?.url}`);
      }
    } finally {
      setLoading(false);
    }
  ...
  // Give an immediate refresh on arrival after booking
  useEffect(() => {
    if (slotIdFromQuery) {
      fetchQueue();
    }
  ...
  const calculateWaitTime = useCallback(() => {
    ...
    const position = userPosition - (currentServing || 0);
    if (position <= 0) return 'Now';
    return `~${position * ESTIMATED_TIME_PER_SLOT} min`;
  }, [userPosition, currentServing]);
              ...
              {slots.map((slot) => {
                ...
                return (
                  <button
                    key={slot.id}
                    className={`w-12 h-12 rounded-full flex items-center justify-center shadow-sm ${spotClass}`}
                    aria-label={`Position ${slot.slot_number + 1} - ${slotStatus}`}
