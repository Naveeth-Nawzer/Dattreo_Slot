...
export default function LanguageSelection() {
  ...
  return (
    ...
    <div className="flex items-center justify-center bg-transparent relative overflow-hidden p-4 sm:p-0">
      <TealWaveBackground/>
      <BrushTealWaves/>
      ...
      <div className="bg-[#F7FBFB] p-6 sm:p-10 md:p-20 rounded-3xl shadow-lg text-center z-10 w-full max-w-md sm:max-w-xl md:max-w-2xl lg:w-[800px] mt-[80px] sm:mt-[60px] md:mt-[120px]">
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 md:gap-10 justify-center">
          {languages.map((lang) => (
            <button
              key={lang}
              className="px-6 py-4 sm:px-10 sm:py-5 md:px-12 md:py-6 rounded-full border border-emerald-300 bg-white text-lg sm:text-xl md:text-2xl text-black font-semibold hover:bg-emerald-50 transition outline outline-1 outline-emerald-300 w-full sm:w-auto"
            >
              {lang}
            </button>
          ))}
